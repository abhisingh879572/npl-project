...
const Navbar = ({ theme, setTheme }) => {
  ...
  return (
    <div
      className="relative z-10 w-full duration-300 shadow-md dark:bg-black dark:text-white"
      style={{ backgroundColor: "#062446", color: "" }}
    >
                ...
                >
                  {subLinks ? (
                    <>
                      ...
                      {showSubMenu === id && (
                        <ul
                          className={`absolute left-0 z-20 py-2 bg-white text-dark rounded-md shadow-md top-full dark:bg-light-300 w-50% col-sm-8 mt-2`}
                        >
                          {subLinks.map((subLink) => (
                            <li key={subLink.id}>
                              <a
                                href={subLink.link}
                                className="block px-12 py-2 text-sm hover:bg-gray-200 dark:hover:bg-white-700 w-50%"
